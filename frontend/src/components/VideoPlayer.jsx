...
export default function VideoPlayer({ stream, isLocal = false, name, isAudioMuted = false, isVideoOff = false }) {
  ...
  return (
    <div style={{ position: 'relative', borderRadius: '1rem', overflow: 'hidden', background: '#000', boxShadow: '0 10px 30px rgba(0,0,0,0.3)', aspectRatio: '16/9' }}>
      ...
      {/* ALWAYS keep the video mounted if stream exists to prevent WebRTC reattachment bugs */}
      {stream && (
        <video
          ref={videoRef}
          autoPlay
          ...
          style={{ 
            ...
            display: isVideoOff ? 'none' : 'block' // Hide instead of unmounting
          }} 
        />
      )}

      {/* Show overlay when video is off or stream is missing */}
      {(!stream || isVideoOff) && (
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#000', color: 'var(--text-secondary)', zIndex: 10 }}>
          {isVideoOff ? (
      ...
      <div style={{ position: 'absolute', bottom: '1rem', left: '1rem', background: 'rgba(0,0,0,0.5)', padding: '0.25rem 0.75rem', borderRadius: '0.5rem', backdropFilter: 'blur(4px)', color: 'white', zIndex: 20 }}>
